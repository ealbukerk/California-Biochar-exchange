...
async function confirmTransactionDelivery(transactionId, confirmerUID) {
  ...
  const updates = {}
  if (tx.buyerUID === confirmerUID) {
    updates.confirmedByBuyer = true
  }
  if (tx.producerUID === confirmerUID) {
    updates.confirmedByProducer = true
  }
  ...
  if (updated.confirmedByBuyer === true && updated.confirmedByProducer === true) {
    ...
    if (dealSnap.exists) {
      ...
      await updateVerifiedStatus(deal.buyerUID)
      if (deal.producerUID) await updateVerifiedStatus(deal.producerUID)
    }
  }
...
async function getDealPartyVerification(deal) {
  const result = {
    buyerVerified: false,
  ...
  if (deal.buyerUID) {
    const buyerDoc = await db.collection('users').doc(deal.buyerUID).get()
    result.buyerVerified = !!(buyerDoc.exists && buyerDoc.data() && buyerDoc.data().verified === true)
  }
  if (deal.producerUID) {
    const producerDoc = await db.collection('users').doc(deal.producerUID).get()
    result.producerVerified = !!(producerDoc.exists && producerDoc.data() && producerDoc.data().verified === true)
  }

  return result
}

function formatDealPartyName(name, isVerified) {
  const safeName = String(name || '')
  if (isVerified && typeof window.renderVerifiedBadge === 'function') {
    return safeName + ' ' + window.renderVerifiedBadge()
  }
  return safeName
}

async function getDealHeaderParties(deal) {
  const verification = await getDealPartyVerification(deal)
  return {
    producerName: formatDealPartyName(deal && deal.producerName, verification.producerVerified),
    buyerName: formatDealPartyName(deal && deal.buyerName, verification.buyerVerified)
  }
}

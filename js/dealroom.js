(function () {
    ...
    container.innerHTML =
      '<div style="display:grid;grid-template-columns:1fr 400px;gap:28px;align-items:start;padding:var(--space-6) 0">' +
        '<div style="display:flex;flex-direction:column;gap:16px">' +
          ...
          '</div>' +
        '</div>' +
        '<div style="position:sticky;top:calc(60px + var(--space-4))"><div id="dr-bid-panel" style="background:var(--color-surface);border:1px solid var(--color-border);border-radius:var(--radius-lg);padding:24px;box-shadow:var(--shadow-sm)"></div></div>' +
      "</div>";
    rendered = true;
  }
  ...
  function renderMessages(state) {
    ...
    thread.innerHTML = state.messages.map(function (msg) {
      ...
      return '<div style="display:flex;flex-direction:column;align-items:' + (isMine ? "flex-end" : "flex-start") + '"><span style="font-size:11px;color:var(--color-text-muted);margin-bottom:2px">' + (msg.senderName || "Unknown") + '</span><div style="background:' + (isMine ? "var(--color-accent)" : "var(--color-bg)") + ";color:" + (isMine ? "white" : "var(--color-text-primary)") + ";padding:10px 14px;border-radius:" + (isMine ? "12px 12px 4px 12px" : "12px 12px 12px 4px") + ';max-width:80%;font-size:14px">' + renderMessageBody(msg) + "</div></div>";
    }).join("");
